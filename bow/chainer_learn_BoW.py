...
def save(model,optimizer):
    ...
    serializers.save_npz('classifier_ffnn_bow.model', model)
    print('save the optimizer')
    serializers.save_npz('classifier_ffnn_bow.state', optimizer)
...
def load_data(f_name,d_name,c_num):
    name2serif,name2id = make_name2serif(f_name,c_num)
    dictionary = corpora.Dictionary.load_from_text(d_name)
    # 特徴抽出
    data_train = []
    label_train = []
...
def main():
    ...
    parser.add_argument('--dict '    , dest='dict'       , type=str, default='dict.dat',  help='an dict file')
